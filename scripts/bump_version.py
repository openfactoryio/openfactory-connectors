...
def bump_version(version: str) -> None:
    ...
    # Update OpenFactory dependency version
    updated = False
    for i, dep in enumerate(toml_doc["project"]["dependencies"]):
        if dep.startswith("OpenFactory @ git+https://github.com/openfactoryio/openfactory-core.git@"):
            toml_doc["project"]["dependencies"][i] = (
                f"OpenFactory @ git+https://github.com/openfactoryio/openfactory-core.git@v{version}"
            )
            updated = True
            break

    if not updated:
        print("ERROR: OpenFactory dependency not found — no update made.")
        sys.exit(1)
    ...
    print(f"✅ OpenFactory dependency pinned to: v{version}")
